...
\begin{center}
	...
	\textbf{Homework 8} - - \textit{all problems now included here} -- \textbf{20 points}\\
\end{center}
...
\begin{enumerate}
	...
	\item Create two Python files patterned after \texttt{ex12\_password.py} and \texttt{call\_password.py} from class, naming your files as you see fit but preface the name of each with \texttt{hw08ex5}. As with the class files, one of your files will contain a \textbf{TUI} that receives input from the user (however you would like) and the other file works as a script (like \texttt{call\_password.py} did) taking the input from the TUI, performing some action with it, and producing output the user can see. Do not reproduce the password example, but otherwise you may create any application you would like. A very simple example would be to have the TUI accept a pair of numbers and a mathematical operation and have the calling Python script perform the operation on the numbers and display the result. A more complicated example would be to have the script run an algorithm to solve a problem (e.g. bin packing problem, linear program, etc.) receiving some or all of its input from the TUI.
